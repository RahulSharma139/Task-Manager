...
const fetchTasks = async () => {
  try {
    const res = await axios.get(`${BASE_URL}/taskmanager/task`);
  ...
  const handleAddTask = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await axios.post(`${BASE_URL}/taskmanager/task`, newTask);
  ...
  const handleSave = async (index: number): Promise<void> => {
    ...
    const updatedTask = { ...taskToUpdate, name: editName , tag: editTag };
  
    try {
      const res = await axios.patch(`${BASE_URL}/taskmanager/task/${taskToUpdate._id}`, updatedTask);
  ...
  const handleDelete = async (index: number): Promise<void> => {
    ...
    try {
      // Make sure to send the correct task ID to the backend
      await axios.delete(`${BASE_URL}/taskmanager/task/${taskToDelete._id}`);
